...
class Book:
    ...
    def scrap_page_one_book(self, url_book_of_one_book):
...
# list of all urls of books from category by browsing each page of the category
    def list_all_pages_of_category(self, url_page_category):
...
# create CSV
    def to_csv(self, datas, category):
    ...
    def from_list_url_to_categories_csv(self, list_url_category):
        ...
        for url_page_category in list_url_category:
            datas = []
            print(url_page_category)
            return_of_list_all_pages_of_catagory = self.list_all_pages_of_category(url_page_category)
            for page_url in return_of_list_all_pages_of_catagory:
                ...
                for values in soup.find_all("h3"):
                    ...
                    datas_of_one_book, category = self.scrap_page_one_book(a_href_remplaced)
                    datas.append(datas_of_one_book) 
                    
            self.to_csv(datas, category)
    



# Scrap all URL of category in home page
    def extract_all_links_categories_from_home_page(self, url):
    ...
    def scrap_all_categories(self, url):
        ...
        Return: none.
        """
        categories = self.extract_all_links_categories_from_home_page(url)
        self.from_list_url_to_categories_csv(categories)
